...
func TestExecGitShell(t *testing.T) {
	...
	stderr, err := execGitShell("git-upload-pack '/the/repo.git'", "sickill")
	if stderr != "" || err != nil {
	...
	expectedStderr := "-c git-upload-pack '/the/repo.git'\nGITORIOUS_PROTO=ssh\nGITORIOUS_USER=sickill"
	stderr, err = execGitShell("git-upload-pack '/the/repo.git'", "sickill")
	if stderr != expectedStderr || err == nil {
		t.Errorf("expected output \"%v\" on stderr or error, got \"%v\"", expectedStderr, stderr)
	}
}
